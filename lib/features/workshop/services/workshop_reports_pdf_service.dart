...
class WorkshopReportsPdfService {
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          pw.Text(
            'Generated ${_date(DateTime.now())} | $filterSummary',
            style: const pw.TextStyle(fontSize: 9),
          ),
  ...
  Future<Uint8List> generateRepairJobs({
    ...
    final estimatedHours = jobs.fold<double>(
      0,
      (total, job) => total + job.estimatedHours,
    );
    final actualHours = jobs.fold<double>(
      0,
      (total, job) => total + job.actualHours,
    );
    final estimatedCost = jobs.fold<double>(
      0,
      (total, job) => total + job.estimatedCost,
    );
    final actualCost = jobs.fold<double>(
      0,
      (total, job) => total + job.actualCost,
    );
    final pdf = pw.Document();
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (_) => [
          ...
          pw.Wrap(
            ...
            runSpacing: 8,
            children: [
              _metric('Total jobs', '${jobs.length}'),
              _metric('Completed jobs', '$completed'),
              _metric('Outstanding jobs', '$outstanding'),
              _metric(
                'Estimated labour hours',
                estimatedHours.toStringAsFixed(1),
              ),
                ...
                children: [
                  pw.Row(
                    children: [
                      pw.Expanded(
                        child: pw.Text(
                          '${job.jobNumber} | ${job.vehicleRegistration}',
                          style: pw.TextStyle(
                            fontSize: 12,
                            fontWeight: pw.FontWeight.bold,
                          ),
                      ...
                      pw.Text(
                        '${_title(job.priority.name)} | ${_title(job.status.name)}',
                        style: const pw.TextStyle(fontSize: 9),
                      ),
                    ],
                  ),
                  ...
                  pw.Wrap(
                    ...
                    children: [
                      pw.Text(
                        'Fleet: ${inspection?.fleetNumber ?? 'Not recorded'}',
                        style: const pw.TextStyle(fontSize: 8),
                      ),
                      pw.Text(
                        'Inspection: ${inspection?.inspectionNumber ?? job.inspectionId}',
                        style: const pw.TextStyle(fontSize: 8),
                      ),
                      pw.Text(
                        'Technician: ${job.technicianId == null ? 'Unassigned' : job.technicianName}',
                        style: const pw.TextStyle(fontSize: 8),
                      ),
                      pw.Text(
                        'Parts required: ${job.partsRequired ? 'Yes' : 'No'}',
                        style: const pw.TextStyle(fontSize: 8),
                      ),
                      pw.Text(
                        'Hours: ${job.estimatedHours.toStringAsFixed(1)} est / ${job.actualHours.toStringAsFixed(1)} actual',
                        style: const pw.TextStyle(fontSize: 8),
                      ),
                      pw.Text(
                        'Cost: ${_money(job.estimatedCost)} est / ${_money(job.actualCost)} actual',
                        style: const pw.TextStyle(fontSize: 8),
                      ),
                      pw.Text(
                        'Created: ${_dateTime(job.createdAt)}',
                        style: const pw.TextStyle(fontSize: 8),
                      ),
                      if (job.startedAt != null)
                        pw.Text(
                          'Started: ${_dateTime(job.startedAt!)}',
                          style: const pw.TextStyle(fontSize: 8),
                        ),
                      if (job.completedAt != null)
                        pw.Text(
                          'Completed: ${_dateTime(job.completedAt!)}',
                          style: const pw.TextStyle(fontSize: 8),
                        ),
                    ],
                  ),
  ...
  Future<Uint8List> generateVehicleHistory({
    ...
    final logo = await _loadLogo();
    final vehicle = history.inspections.isEmpty
        ? null
        : history.inspections.first;
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (_) => [
          pw.Text(
            '${vehicle?.registration ?? 'Vehicle not recorded'} | Fleet ${vehicle?.fleetNumber ?? 'Not recorded'}',
          ...
          pw.Wrap(
            ...
            children: [
              ...
              _metric(
                'Failed / repair items',
                '${history.failedOrRepairRequiredItems}',
              ),
              ...
              _metric(
                'Actual labour hours',
                history.actualHours.toStringAsFixed(1),
              ),
          ...
          ...history.inspections.map((inspection) {
            final jobs = inspection.id == null
                ? const <RepairJob>[]
                : jobsByInspection[inspection.id] ?? const <RepairJob>[];
            return pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  pw.Text(
                    '${_dateTime(inspection.dateStarted)} | ${inspection.inspectionNumber}',
                    style: pw.TextStyle(
                      fontSize: 11,
                      fontWeight: pw.FontWeight.bold,
                    ),
                  ...
                  pw.Text(
                    '${inspection.inspectionType.label} | ${_title(inspection.status.name)} | Sign-off: ${inspection.managerSignature?.trim().isNotEmpty == true ? 'Recorded' : 'Not recorded'}',
                    style: const pw.TextStyle(fontSize: 9),
                  ),
                  pw.Text(
                    'Saved failed / repair-required items: ${inspection.criticalFailures + inspection.repairsRequired}',
                    style: const pw.TextStyle(fontSize: 9),
                  ),
                  if (jobs.isEmpty)
                    pw.Text(
                      'No linked repair jobs.',
                      style: const pw.TextStyle(fontSize: 9),
                    )
                  else
                    ...jobs.map(
                      (job) => pw.Padding(
                        ...
                        child: pw.Text(
                          '${job.jobNumber} | ${job.title} | ${job.technicianId == null ? 'Unassigned' : job.technicianName} | ${_title(job.status.name)} | ${job.actualHours.toStringAsFixed(1)} hrs | ${_money(job.actualCost)}${job.completedAt == null ? '' : ' | Completed ${_dateTime(job.completedAt!)}'}',
                          style: const pw.TextStyle(fontSize: 8),
                        ),
                      ),
                    ),
      ...
      pw.MultiPage(
        ...
        build: (_) => [
          ...
          pw.Wrap(
            ...
            runSpacing: 8,
            children: [
              _metric('Assigned jobs', '${data?.jobs.length ?? 0}'),
              _metric('Active jobs', '${data?.activeJobs ?? 0}'),
              _metric('Awaiting review', '${data?.awaitingReview ?? 0}'),
              _metric('Completed jobs', '${data?.completedJobs ?? 0}'),
              _metric('Vehicles worked on', '${data?.vehicleCount ?? 0}'),
              _metric(
                'Actual labour hours',
                (data?.actualHours ?? 0).toStringAsFixed(1),
              ),
              _metric('Actual cost recorded', _money(data?.actualCost ?? 0)),
            ],
          ),
          pw.SizedBox(height: 16),
          pw.Text(
          ...
          ...(data?.jobs ?? const <RepairJob>[]).map(
            (job) => pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  pw.Text(
                    '${job.jobNumber} | ${job.vehicleRegistration} | ${_title(job.status.name)}',
                    style: pw.TextStyle(
                      fontSize: 11,
                      fontWeight: pw.FontWeight.bold,
                    ),
                  ...
                  pw.Wrap(
                    ...
                    children: [
                      ...
                      pw.Text(
                        'Priority: ${_title(job.priority.name)}',
                        style: const pw.TextStyle(fontSize: 8),
                      ),
                      pw.Text(
                        'Actual hours: ${job.actualHours.toStringAsFixed(1)}',
                        style: const pw.TextStyle(fontSize: 8),
                      ),
                      pw.Text(
                        'Actual cost: ${_money(job.actualCost)}',
                        style: const pw.TextStyle(fontSize: 8),
                      ),
                      pw.Text(
                        'Created: ${_dateTime(job.createdAt)}',
      ...
      pw.MultiPage(
        ...
        build: (_) => [
          ...
          pw.Wrap(
            ...
            runSpacing: 8,
            children: [
              _metric('Total repair jobs', '${summary.jobs.length}'),
              _metric('Completed jobs', '${summary.completedJobs}'),
              _metric('Outstanding jobs', '${summary.outstandingJobs}'),
              _metric('Estimated cost', _money(summary.estimatedCost)),
              _metric('Actual cost', _money(summary.actualCost)),
              _metric('Cost variance', _money(summary.costVariance)),
              _metric(
                'Estimated labour hours',
                summary.estimatedHours.toStringAsFixed(1),
              ),
              _metric(
                'Actual labour hours',
                summary.actualHours.toStringAsFixed(1),
              ),
            ],
          ),
          pw.SizedBox(height: 16),
          ...
          pw.TableHelper.fromTextArray(
            headers: const ['Vehicle', 'Jobs', 'Actual cost'],
            data: summary.actualCostByVehicle.entries
                .map(
                  (entry) => [
                    entry.key,
                    '${vehicleCounts[entry.key] ?? 0}',
                    _money(entry.value),
                  ],
                )
                .toList(),
          ...
          pw.TableHelper.fromTextArray(
            headers: const ['Priority', 'Jobs', 'Actual cost'],
            data: summary.actualCostByPriority.entries
                .map(
                  (entry) => [
                    _title(entry.key),
                    '${priorityCounts[entry.key] ?? 0}',
                    _money(entry.value),
                  ],
                )
                .toList(),
      ...
      pw.MultiPage(
        ...
        build: (_) => [
              ...
              pw.Expanded(
                child: pw.Column(
                  ...
                  children: [
                    ...
                    pw.Text(inspection.inspectionNumber),
                  ],
                ),
              ),
            ],
          ...
          _detailSection('Inspection summary', [
            ...
            _detailRow(
              'Template / type',
              inspection.templateName?.trim().isNotEmpty == true
                  ? inspection.templateName!
                  : inspection.inspectionType.label,
            ),
            _detailRow(
              'Result / status',
              '${_title(inspection.overallResult.name)} / ${_title(inspection.status.name)}',
            ),
          ...
          _detailSection('Vehicle and people', [
            ...
            _detailRow(
              ...
              inspection.technicianName.trim().isEmpty
                  ? 'Not recorded'
            ...
            data: report.items
                .map(
                  (item) => [
                    ...
                    item.title,
                    item.responseValue ?? '',
                    _title(item.status.name),
                    item.repairRequired ? 'Required' : 'No',
                    item.notes,
                  ],
                )
                .toList(),
          ...
          if (report.jobs.isEmpty)
            pw.Text('No repair jobs recorded.')
          else
            pw.TableHelper.fromTextArray(
              headers: const [
                'Job',
              ...
              data: report.jobs
                  .map(
                    (job) => [
                      job.jobNumber,
                      itemsById[job.inspectionItemId]?.title ?? 'Not available',
          ...
          if (report.photos.isEmpty)
            ...
            pw.Text(
              'Inspection evidence is recorded, but the local photo file is unavailable.',
            )
          else
            pw.Wrap(spacing: 8, runSpacing: 8, children: evidence),
          _detailSection('Final status / sign-off', [
            _detailRow('Inspection status', _title(inspection.status.name)),
            _detailRow(
              'Manager signature',
              inspection.managerSignature?.trim().isNotEmpty == true
                  ? inspection.managerSignature!
                  : 'Not signed off',
            ),
            _detailRow('Notes', inspection.notes),
          ]),
        ],
      ),
    );
  ...
  pw.Widget _detailSection(String title, List<pw.Widget> children) =>
      pw.Container(
  ...
  pw.Widget _inspectionsTable(List<WorkshopInspection> inspections) =>
      pw.TableHelper.fromTextArray(
        headers: const [
          'Inspection',
          'Vehicle',
        ...
        data: inspections
            .map(
              (inspection) => [
                ...
                '${inspection.registration} / ${inspection.fleetNumber}',
                inspection.inspectionType.label,
                inspection.driverName?.isNotEmpty == true
                    ? inspection.driverName!
                    : inspection.technicianName,
  ...
  static String _title(String value) => value
      .replaceAllMapped(RegExp(r'(?<=[a-z])(?=[A-Z])|_'), (match) => ' ')
      .split(' ')
      .map(
        (part) =>
