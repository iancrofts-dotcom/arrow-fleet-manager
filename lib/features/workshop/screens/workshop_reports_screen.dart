...
class _WorkshopReportsScreenState extends State<WorkshopReportsScreen> {
  ...
  final _operationGate = ReportOperationGate();
  ...
  WorkshopReportFilter _filter() {
    ...
    final range = switch (_period) {
      ...
      _ReportPeriod.last7Days => DateTimeRange(
        start: today.subtract(const Duration(days: 6)),
        end: today,
      ),
      _ReportPeriod.last30Days => DateTimeRange(
  ...
  Future<void> _selectCustomRange() async {
    ...
    if (selected.start.isAfter(selected.end)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('The start date must not be after the end date.'),
        ),
      );
      return;
    }
  ...
  Future<Uint8List> _createPdf(_ReportPageData data) async {
    ...
    return switch (_type) {
      ...
      _WorkshopReportType.vehicleHistory => _createVehicleHistoryPdf(
        data,
        jobs,
        inspections,
      ),
  ...
  Future<void> _preview(_ReportPageData data) => _runReportOperation(
  ...
  Future<void> _printOrSave(_ReportPageData data) => _runReportOperation(
    failureMessage: 'Unable to print or save report.',
    operation: () async {
      final bytes = await _createPdf(data);
      if (!mounted) return;
  ...
  Future<void> _exportCsv(_ReportPageData data) => _runReportOperation(
    failureMessage: 'Unable to export report.',
    operation: () async {
        ...
        return [
          ...
          ...jobs.map(
            (job) => row([
              ...
              job.title,
              job.technicianId ?? '',
              _technicianLabel(data, job.technicianId),
              job.priority.name,
              job.status.name,
              job.partsRequired,
              job.estimatedHours,
  ...
  Widget build(BuildContext context) {
    ...
    return AppPageScaffold(
      ...
      subtitle:
          'Operational repair, inspection, technician and cost reporting.',
  ...
  Widget _content(_ReportPageData data) {
    ...
    final vehicles = <int, WorkshopInspection>{
      for (final inspection in data.source.inspections)
        inspection.vehicleId: inspection,
  ...
  Widget _filters(
    _ReportPageData data,
    List<WorkshopInspection> vehicles,
  ) => Wrap(
    spacing: 12,
    runSpacing: 12,
    children: [
      ...
      if (_period == _ReportPeriod.custom)
        OutlinedButton.icon(
          ...
          label: Text(_period.label(_customRange)),
        ),
      ...
      if (_type == _WorkshopReportType.technicianWork ||
          _type == _WorkshopReportType.repairJobs)
        _nullableDropdown<String>(
          'Technician',
          _technicianId,
          data.technicians
              .map((user) => _Option(user.id, user.username))
              .toList(),
          (value) => setState(() => _technicianId = value),
        ),
      if (_type == _WorkshopReportType.repairJobs ||
          _type == _WorkshopReportType.costs)
        _nullableDropdown(
          'Status',
          _status,
          RepairJobStatus.values
              .map((value) => _Option(value, _readable(value.name)))
              .toList(),
      ...
      if (_type == _WorkshopReportType.inspection)
        _nullableDropdown<int>(
          ...
          data.source.inspections
              .where((inspection) => inspection.id != null)
              .map(
                (inspection) => _Option(
                  inspection.id!,
              ...
              .toList(),
          (value) => setState(() => _inspectionId = value),
        ),
    ],
  );
  ...
  Widget _summary(
    ...
    if (_type == _WorkshopReportType.vehicleHistory && _vehicleId == null) {
      return const Text('Select a vehicle to view its Workshop history.');
    }
    if (_type == _WorkshopReportType.technicianWork && _technicianId == null) {
      return const Text('Select a Technician account to view assigned work.');
    }
    if (_type == _WorkshopReportType.inspection && _inspectionId == null) {
      return const Text(
        'Select a Workshop inspection to create its detailed report.',
      );
    }
    if (_type == _WorkshopReportType.vehicleHistory) {
      ...
      return FutureBuilder<VehicleWorkshopHistory>(
        ...
        builder: (context, snapshot) {
          if (!snapshot.hasData) {
            return const Text('Loading vehicle history summary...');
          }
    ...
    final values = switch (_type) {
      _WorkshopReportType.repairJobs => {
  ...
  Map<String, Object> _technicianSummary(List<RepairJob> jobs) {
    ...
    return {
      'Assigned': summary.jobs.length,
      'Active': summary.activeJobs,
      'Awaiting review': summary.awaitingReview,
      'Completed': summary.completedJobs,
      'Vehicles': summary.vehicleCount,
      'Actual hours': summary.actualHours.toStringAsFixed(1),
      'Actual cost': _money(summary.actualCost),
    };
  }
  ...
  Map<String, Object> _costSummary(List<RepairJob> jobs) {
    ...
    return {
      ...
      'Completed': summary.completedJobs,
      'Outstanding': summary.outstandingJobs,
      'Estimated cost': _money(summary.estimatedCost),
      'Actual cost': _money(summary.actualCost),
      'Variance': _money(summary.costVariance),
      'Estimated hours': summary.estimatedHours.toStringAsFixed(1),
      'Actual hours': summary.actualHours.toStringAsFixed(1),
    };
  }
  ...
  Widget _results(
    ...
    if (_type == _WorkshopReportType.inspection) {
      return const AppEmptyState(
        ...
        message:
            'Use the selected inspection to preview or save a structured report.',
      );
    }
    if (_type == _WorkshopReportType.vehicleHistory && _vehicleId == null) {
    ...
    if (jobs.isEmpty && inspections.isEmpty) {
      return const AppEmptyState(
        title: 'No matching Workshop records',
        message: 'Adjust the selected report filters.',
      );
    }
    return SectionCard(
      title: 'Results',
  ...
  static String _typeLabel(_WorkshopReportType value) => switch (value) {
    ...
    _WorkshopReportType.inspection => 'Inspection Report',
  };
...
class _ReportPageData {
  const _ReportPageData({required this.source, required this.technicians});
  final WorkshopReportSource source;
...
class _WorkshopReportsAccessDenied extends StatelessWidget {
  const _WorkshopReportsAccessDenied();
  @override
  Widget build(BuildContext context) => const Scaffold(
    body: AppEmptyState(
      title: 'Access denied',
      message: 'Workshop reports are available to Workshop management only.',
    ),
  );
}
