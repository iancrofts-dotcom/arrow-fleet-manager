...
class RepairRepository {
  Future<Database> get _db async => await DatabaseService().database.database();
  ...
  Future<Repair?> getRepair(int id) async {
    ...
    return Repair.fromMap(result.first);
  }

  Future<List<Repair>> getRepairs() async {
    final db = await _db;

    final result = await db.query('repairs', orderBy: 'dateRaised DESC');
  ...
  Future<WorkshopSummary> getWorkshopSummary() async {
    final openRepairs = await getOpenRepairCount();

    final highPriority = await getHighPriorityCount();

    final overdueRepairs = await getOverdueRepairCount();

    final completedThisWeek = await getCompletedThisWeekCount();
  ...
  Future<List<RepairTrend>> getWeeklyRepairTrends() async {
    ...
    for (int i = 5; i >= 0; i--) {
      ...
      trends.add(
        RepairTrend(period: 'W${6 - i}', opened: opened, completed: completed),
      );
    }
