...
class _EditRepairScreenState extends State<EditRepairScreen> {
  ...
  void initState() {
    ...
    _mechanicController = TextEditingController(text: widget.repair.mechanic);

    _notesController = TextEditingController(text: widget.repair.repairNotes);
  ...
  Future<void> _saveRepair() async {
    final updatedRepair = widget.repair.copyWith(
      ...
      repairNotes: _notesController.text.trim(),
      status: _status,
      priority: _priority,
      completedDate: _status == 'Completed' ? DateTime.now() : null,
    );

    await _repository.updateRepair(updatedRepair);
