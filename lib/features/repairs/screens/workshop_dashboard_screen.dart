...
class _WorkshopDashboardScreenState extends State<WorkshopDashboardScreen> {
  ...
  void loadRepairs() {
    repairsFuture = _repairRepository.getOpenRepairs();

    summaryFuture = _repairRepository.getWorkshopSummary();

    trendsFuture = _repairRepository.getWeeklyRepairTrends();
  }
  ...
  void addRepair() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Manual repair creation will be added in Sprint 9.5.'),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Workshop Dashboard')),
            ...
            child: ListView(
              ...
              children: [
                FutureBuilder<WorkshopSummary>(
                  ...
                  builder: (context, snapshot) {
                    final summary = snapshot.data ?? WorkshopSummary.empty();
                    ...
                    if (snapshot.hasError) {
                      return Card(
                        child: Padding(
                          ...
                          child: Text('Unable to load repair trends.'),
                        ),
                      );
                    }
