...
class AssignmentHistoryScreen extends StatefulWidget {
  const AssignmentHistoryScreen({super.key, required this.vehicleId});
...
class _AssignmentHistoryScreenState extends State<AssignmentHistoryScreen> {
  ...
  Future<void> _loadHistory() async {
    final history = await _repository.getVehicleAssignmentHistory(
      widget.vehicleId,
    );
  ...
  Widget build(BuildContext context) {
    return AppPageScaffold(
      ...
      subtitle: 'Current and ended driver assignments.',
      child: _loading
          ? const AppLoadingState(label: 'Loading assignment history...')
          : _history.isEmpty
          ? const AppEmptyState(
              icon: Icons.history_toggle_off_outlined,
              title: 'No assignment history',
              message: 'Assignments for this vehicle will appear here.',
            )
          : ListView.builder(
