...
class _MaintenanceListScreenState extends State<MaintenanceListScreen> {
  ...
  void _loadRecords() {
    _future = _service.getForVehicle(widget.vehicleId);
  }
  ...
  Future<void> _addMaintenance() async {
    final refresh = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        builder: (_) => AddMaintenanceScreen(vehicleId: widget.vehicleId),
      ),
    );
  ...
  Future<void> _editMaintenance(MaintenanceRecord record) async {
    final refresh = await Navigator.push<bool>(
      context,
      MaterialPageRoute(builder: (_) => EditMaintenanceScreen(record: record)),
    );
